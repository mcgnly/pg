...
export default function SingleTweetFeed(){
  ...
  return (  
    <div>
      <Tweet key={data.index} text={data.text} author={data.author} created={data.created} url={data.url} />
      {replies.map((item, index)=>(
          <Tweet key={'reply'+item.index} text={item.text} author={item.author} created={item.created} url={item.url} />
        ))}
    </div>
    )
}